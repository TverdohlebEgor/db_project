...
function Rimborso({ rimborsoBase, onRimborsoAggiornato }) {
  ...
  const { idRimborso, importo, data, approvato, valutaSimbolo, valutaNome, comunicazione } = rimborsoCompleto;
  ...
  return (
    <div className="rimborso border p-3 mb-3 rounded shadow-sm bg-white">
      ...
      <p><b>Importo:</b> {importoFormattato}</p>
      <p><b>Data:</b> {new Date(data).toLocaleDateString()}</p>
      <p><b>Stato:</b> {statoTesto}</p>

      {comunicazione && <Comunicazione comunicazione={comunicazione} />}

      {approvato === null && (
        <div className="mt-3">
          <button className="btn btn-success me-2" onClick={() => handleDecision(true)}>Accetta</button>
          <button className="btn btn-danger" onClick={() => handleDecision(false)}>Rifiuta</button>
        </div>
      )}
    </div>
  );
